...
validateEnv([
  ...
  { name: 'NODE_ENV', required: true, validate: validators.oneOf(['development', 'production', 'test']), message: 'NODE_ENV must be either development, production or test' },
  { name: 'ADMIN_USERNAME', required: true, validate: validators.requiredNonEmpty, message: 'ADMIN_USERNAME must be set' },
  { name: 'ADMIN_PASSWORD', required: true, validate: validators.minLength(12), message: 'ADMIN_PASSWORD must be set and at least 12 characters', sensitive: true }
]);
