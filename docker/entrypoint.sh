...
if [ ! -f "$DB_PATH" ]; then
  ...
  node --experimental-strip-types /app/scripts/init-db.js
  echo "✓ Database initialized successfully"
else

  echo "Running db migrations..."
  node --experimental-strip-types /app/scripts/migrate-db.js
  
  echo "Validating db schema..."
  if node --experimental-strip-types /app/scripts/validate-db.js; then
...
SERVER_MODE=all node --experimental-strip-types /app/src/server/index.js &
SERVER_PID=$!
